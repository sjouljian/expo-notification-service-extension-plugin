...
`;
     ...
     */
    addPostInstallHook() {
        Log_1.Log.log('[PodfileManager] Adding post_install hook to fix duplicate framework builds');
    ...
    installer.pods_project.targets.each do |target|
      if target.name == 'Pods-NotificationServiceExtension'
        ...
        if (postInstallRegex.test(podfileContent)) {
            Log_1.Log.log('[PodfileManager] Found existing post_install block, injecting framework fix');
            ...
            if (postInstallEnd !== -1) {
                ...
                if (postInstallContent.includes('BUILD_LIBRARY_FOR_DISTRIBUTION')) {
                    Log_1.Log.log('[PodfileManager] Framework fix already present in post_install');
                    return;
                }
                ...
                Log_1.Log.log('[PodfileManager] ✅ Injected framework fix into existing post_install');
            }
            else {
                Log_1.Log.log('[PodfileManager] ⚠️ Could not find end of post_install block');
            }
        }
        else {
            Log_1.Log.log('[PodfileManager] No existing post_install found, plugin post_install will be added at end');
...
`;
            ...
            Log_1.Log.log('[PodfileManager] ✅ Added new post_install block with framework fix');
        }
    }
}
